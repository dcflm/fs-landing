...
export const StaggerTestimonials = () => {
  ...
  return (
    <div
      id="testimonials"
      className="relative w-full overflow-hidden bg-white my-44"
      style={{
...
const TestimonialCard = ({
  ...
  return (
    <motion.div
      ...
      className={`
      absolute left-1/2 top-1/2 cursor-pointer border-[#FF3D00] p-8 text-black transition-colors duration-500 ${isActive ? "z-10 bg-white" : "z-0 bg-black"
        }
      `}
      style={{
      ...
      animate={{
        ...
        boxShadow: isActive ? "0px 8px 0px 4px [#FF3D00]" : "0px 0px 0px 0px [#FF3D00]",
      }}
      transition={{
        type: "spring",
        mass: 3,
    ...
    >
      <span
        className="absolute block origin-top-right rotate-45 bg-[#FF3D00] object-cover"
        style={{
          right: -BORDER_SIZE,
      ...
      />
      <h3
        className={`text-base sm:text-xl ${isActive ? "text-black" : "text-white"
          }`}
      >
        "{testimonial.testimonial}"
      </h3>
      <p
        className={`absolute bottom-8 left-8 right-8 mt-2 text-sm italic ${isActive ? "text-black" : "text-neutral-400"
          }`}
      >
        - {testimonial.by}
      </p>
